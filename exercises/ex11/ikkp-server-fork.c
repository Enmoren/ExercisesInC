...
null pointer, the whole server will not be taken down. Since each child process
doesn't share same address space as well as significant part of memory with the
parent process, so the parent process will not be influenced.
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        if (!fork()){       //create the child process, if fork() call return 0, the child process will run
          close(listener_d);//close the secondary socket
          ...
          close(connect_d); //once the conversation's over, close the socket to the client
          exit(0);          //exit the program once the child process exit
        }
        close(connect_d);
    }
    return 0;
